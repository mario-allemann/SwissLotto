...
public class LotteryTicketController extends Controller<LotteryTicketModel, LotteryTicketView> {
	...
	/**Adds ActionEvents on buttons to change color and lock them if too many get selected
	 * 
	 * @param bArr An ArrayList with buttons
	 * @param count How many buttons have been clicked on
	 * @param maxClickableButtons The amount of buttons which can be clicked before they get locked
	 * @param numbers A container for the user selection
	 * @param areButtonsLocked true if the buttons are disabled, false if they are activated
	 */
	public void addActionEvent(ArrayList<Button> bArr, AtomicInteger count, int maxClickableButtons, TreeSet<Integer> numbers, AtomicBoolean areButtonsLocked) {
		for (Button b : bArr) {
	...
	//Checks if the required amount of buttons got selected in order to start the draw (unlock the play Button)
